<template>
  <div>
    ...
    >
      CURRENT LAST ID: <b>{{ lastPostID }}</b>
    </p>
    <p>
      ID GREW BY <b>{{ delta }}</b>
    </p>
    <p>
      CURRENT POSTS-PER-SECOND: <b>{{ postsPerSecond }}</b>
    </p>
    <p>
      TIME IT TOOK TO UPDATE: <b>{{ timeBetweenUpdates }} SECONDS</b>
    </p>
    <p>
      TIME PASSED SINCE LAST UPDATE: <b>{{ timePassedSeconds }}</b>
    </p>
    <p>
      CHANCE OF FIVE: <b>{{ chanceOf5 }}%</b>
    </p>
    <label for="toggle">Toggle Interval</label>
    <input name="toggle" type="checkbox" @change="toggleInterval()" />
  </div>
</template>
